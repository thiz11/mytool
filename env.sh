...
EOF
    ...
    local A
    A=""
    for i in `cat $T/env.sh | sed -n "/^function /s/function \([a-z_]*\).*/\1/p" | sort`; do
      A="$A $i"
    done
    echo $A
}
...
for f in `test -d device && find -L device -maxdepth 10 -name 'vendorsetup.sh' 2> /dev/null` \
         `test -d vendor && find -L vendor -maxdepth 10 -name 'vendorsetup.sh' 2> /dev/null`
do
    echo "including $f"
    . $f
done
